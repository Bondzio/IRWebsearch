...
/*
 *
 * This used to be good (well, OK) code but it had huge performance problems.
 * So now the code is horrible but it works well enough.
 * 
 * This is just the copy (because I had to rewrite the MeasureCalculator for all tests).
 *
 */
public class CopyOfMeasureCalculator {
	...
	public static void main(String[] args) throws Exception {
		...
		File file = new File("C:/users/Jonathan/Desktop/Log/3.txt");

		FileWriter logger = new FileWriter(file);	
		
		server = new HttpSolrServer("http://localhost:8983/solr/filterChange2");
		
		for(int contentBoost = 6; contentBoost < 7; contentBoost += 5) {
			for(int titleBoost = 6; titleBoost < 7; titleBoost += 5) {
				for(int urlBoost = 16; urlBoost < 17; urlBoost += 5) {
					for(int importantBoost = 41; importantBoost < 42; importantBoost += 5) {
						for(int i = 0; i < 24; i++) {
							...
							logger.write("ContentBoost: " + contentBoost + "; titleBoost: " + titleBoost+ "; urlBoost: " + urlBoost + "; importantBoost: " + importantBoost + "; filter: " + i +"\n");
							logger.write("\tP@10: " + Math.round(averagePrecisionAtTen/queryCounterPAt10 * 100.0) / 100.0);
							logger.write("\tP@1: " + Math.round(averagePrecisionAtOne/queryCounterPAt1 * 100.0) / 100.0);
							logger.write("\tMAP: " + Math.round(averageMAP / queryCounterMAP * 100.0) / 100.0);
							logger.write("\tMRR: " + Math.round(averageMRR / queryCounterMRR * 100.0) / 100.0);
