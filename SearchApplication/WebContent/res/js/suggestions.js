...
function initSuggestions() {	
	...
	searchbox = $("#query-field");
	searchbox.autocomplete();
	searchbox.autocomplete("enable");
	searchbox.on("click", _onSearchboxClick);
	searchbox.on("input", _onSearchboxInput);
}
...
function _onSearchboxClick() {
	callback({suggestions: {length:0}});
}
...
function callback(data) {
	...
	for(var i = 0; i < pastQueries.length; i++) {
		if(suggestionsArray.length <= 4 && beginsWith(pastQueries[i], searchbox.val())) {
 ...
 *
 */
function _onSearchboxInput(event) { 
	var url = "WebAppTest";
